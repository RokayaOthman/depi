const ProductCard = ({ className }) => {
  return (
    <div
      className={`max-w-fit text-left bg-[#F9F9F9] p-6 font-sans text-gray-900 ${className}`}>
      {/* Header: Title & Price */}
  ...
  <div className="flex gap-4">
    {[
      { color: "#707477", name: "Grey" },
      { color: "#5B1F1F", name: "Maroon" },
      { color: "#2C2E33", name: "Green", active: true },
      { color: "#E8DCC4", name: "Beige" }, // custom hex color
    ].map((item) => (
      <button
        key={item.name}
